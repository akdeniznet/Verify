...
class DiziBox : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = if (request.name == "Dizi Arşivi") {
            document.select("article.detailed-article").mapNotNull { it.toMainPageResult() }
        } else {
            document.select("article.article-series-poster").mapNotNull { it.toMainPageResult() }
        }
    ...
    private suspend fun iframeDecode(
        ...
        try {
            if (iframe.contains("/player/king/king.php")) {
                ...
                return true
            } else if (iframe.contains("/player/moly/moly.php") || iframe.contains("/player/haydi.php")) {
                val phpFile = if (iframe.contains("moly.php")) "moly.php" else "haydi.php"
                iframe = iframe.replace("$phpFile?", "$phpFile?wmode=opaque&")
                var subDoc = app.get(
                    iframe,
                    referer = data,
                    cookies = mapOf(
